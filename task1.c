...
int main() // This function will return an integer
{
    ...
    {
        printf("%s scores 3 points and %s scores 0 points", nameTeam1, nameTeam2 ); //If the statement is true, since the first team has a higher score, print that they score 3 points and the second team scores 0
    }
      ...
      if (team1Score < team2Score) //Compares the two vales to see if the second team's score value is higher than the team's
        {
            printf("%s scores 3 points and %s scores 0 points", nameTeam2, nameTeam1 ); //Print that the second team get 3 points and the first team gets 0
        }
        else //If neither statements are met then both teams must have the same score
    {
            printf("Both %s and %s score 1 point", nameTeam1, nameTeam2); //Since it must be a draw, both teams recieve 1 point
    }
